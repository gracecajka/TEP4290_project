...
import numpy as np
import scipy.stats
from typing import Container, Dict, Optional, Union # for type checking in methods
...
class DynamicStockModel(object):
    ...
    def __init__(self, 
                 t: Optional[Container] = None, 
                 i: Optional[np.ndarray] = None, 
                 i_c: Optional[np.ndarray] = None, 
                 o: Optional[np.ndarray] = None, 
                 s: Optional[np.ndarray] = None, 
                 lt: Optional[Dict[str, Union[int, np.ndarray]]] = None, 
                 s_c: Optional[np.ndarray] = None, 
                 o_c: Optional[np.ndarray] = None, 
                 name: str = 'DSM', 
    ...
    def compute_s_c_from_i_c(self): #Added by Fernando
        ...
        if self.i_c is not None:
            if self.lt is not None:
                self.compute_sf()
                self.s_c = np.matmul( self.sf, self.i_c) # The einstein summation becomes a simple matrix multiplication
                # This command means: s_c[t,c] = i[t,c] * sf[t,c] for all t, c
                # from the perspective of the stock the inflow has the dimension age-cohort, 
                # as each inflow(t) is added to the age-cohort c = t
                return self.s_c
            else:
